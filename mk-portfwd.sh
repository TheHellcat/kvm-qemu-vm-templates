...
# UDP forward
iptables -t nat -A PREROUTING -p udp -d $HOSTIP --dport $SRCPORT -j DNAT --to $DSTADDR:$DSTPORT
iptables -I FORWARD -d $DSTADDR/32 -p udp -m state --state NEW -m udp --dport $DSTPORT -j ACCEPT
